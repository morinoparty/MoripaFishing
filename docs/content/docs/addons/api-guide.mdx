...
| `WorldLifecycleProvider?` | `getWorldLifecycleProvider()` | ワールド生成・境界管理の Integration (未導入時は `null`) |
...
ワールド生成は [`MoripaFishing-Integration-WorldLifecycle` Integration](../integration/world-lifecycle) が提供する。
未導入環境では `getWorldLifecycleProvider()` が `null` を返すのでチェックが必要。
...
worldLifecycle.addGenerator(myCustomGenerator)
```

...
export class WeatherRandom {
  ...
  constructor(fishingWorldId: string, pepper: string, weight: { x: number, weather: WeatherType }[], maxInclination: number) {
  ...
  async get(x: number): Promise<number> {
    ...
    //small = 6 - 6 % 5 = 5
    //large = 6 + (5 - 6 % 5) + 1 = 10
    const small = (new Random(await this.getHash(x - (x % check)))).nextInt(0, 100)
    const large = (new Random(await this.getHash(x - (x % check) + check))).nextInt(0, 100)
  ...
  private async getHash(x: number) {
    const hash = await this.sha256(this.fishingWorldId + this.pepper + x.toString());
    console.log("x: " + x + " hash: " + parseInt(hash.slice(0, 8), 16))
